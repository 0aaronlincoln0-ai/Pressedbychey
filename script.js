...
const customerDeleteConversation = document.querySelector("#customerDeleteConversation");
const customerEmojiButton = document.querySelector("#customerEmojiButton");
...
const adminEmojiButton = document.querySelector("#adminEmojiButton");
const adminEmojiPicker = document.querySelector("#adminEmojiPicker");
...
const orderPaymentFilter = document.querySelector("#orderPaymentFilter");
...
function messageConversationKey(conversation) {
  const messages = Array.isArray(conversation?.messages) ? conversation.messages : [];
...
function renderMessageThread(thread, conversation, emptyLabel, viewer, contextKey = "") {
...
function renderCustomerMessages() {
  ...
  const emptyLabel = hasAccess ? "Send Chey a message about your order or design." : "Sign in again to load your live conversation.";
  if (user) {
    renderMessageThread(customerMessageThread, customerConversation, emptyLabel, "customer", user.email);
  } else {
    const signedOutKey = "signed-out|customer";
    if (customerMessageThread.dataset.renderKey !== signedOutKey) {
      customerMessageThread.innerHTML = `<div class="message-thread-empty"><strong>Sign in to message Chey</strong><span>Your customer account keeps your conversation and order updates together.</span></div>`;
      customerMessageThread.dataset.renderKey = signedOutKey;
    }
  }
  if (customerMessageInput) {
    customerMessageInput.disabled = !hasAccess;
    customerMessageInput.placeholder = hasAccess ? "Reply..." : "Reply...";
  }
  if (customerMessageSend) customerMessageSend.disabled = !hasAccess;
  if (customerDeleteConversation) customerDeleteConversation.disabled = !hasAccess;
...
function insertMessageEmoji(input, emoji) {
  if (!input) return;
  const start = input.selectionStart ?? input.value.length;
  const end = input.selectionEnd ?? start;
  input.value = `${input.value.slice(0, start)}${emoji}${input.value.slice(end)}`.slice(0, Number(input.maxLength) || 2400);
  input.focus();
  const cursor = Math.min(start + emoji.length, input.value.length);
  input.setSelectionRange(cursor, cursor);
  input.dispatchEvent(new Event("input", { bubbles: true }));
}

function closeMessageEmojiPickers() {
  document.querySelectorAll(".chat-emoji-picker:not([hidden])").forEach((picker) => {
    picker.hidden = true;
    picker.previousElementSibling?.setAttribute("aria-expanded", "false");
  });
}

function setupMessageEmojiPicker(button, picker, input) {
  if (!button || !picker || !input) return;
  picker.innerHTML = MESSAGE_EMOJIS.map((emoji) => `<button type="button" role="menuitem" data-message-emoji="${escapeAttribute(emoji)}" aria-label="Add ${escapeAttribute(emoji)}">${emoji}</button>`).join("");
  button.addEventListener("click", (event) => {
    event.stopPropagation();
    const shouldOpen = picker.hidden;
    closeMessageEmojiPickers();
  ...
  picker.addEventListener("click", (event) => {
    ...
    insertMessageEmoji(input, emojiButton.dataset.messageEmoji || "");
    picker.hidden = true;
    button.setAttribute("aria-expanded", "false");
  });
}

async function undoCustomerMessage(messageId, button) {
  if (!messageId || !customerMessagePassword()) return;
  if (button) button.disabled = true;
  try {
    const data = await customerMessagesRequest("delete-message", { messageId });
    customerConversation = data.conversation || customerConversation;
    renderCustomerMessages();
    if (customerMessagesStatus) customerMessagesStatus.textContent = "Message undone";
  } catch (error) {
    if (customerMessagesStatus) customerMessagesStatus.textContent = error.message || "Message could not be undone.";
    if (button) button.disabled = false;
  }
}

async function undoAdminMessage(messageId, button) {
  if (!messageId || !activeAdminMessageEmail) return;
  if (button) button.disabled = true;
  try {
    const data = await adminMessagesRequest("delete-message", { email: activeAdminMessageEmail, messageId });
    activeAdminConversation = data.conversation || activeAdminConversation;
    renderAdminMessageThread();
    if (adminMessagesStatus) adminMessagesStatus.textContent = "Reply undone";
  } catch (error) {
    if (adminMessagesStatus) adminMessagesStatus.textContent = error.message || "Reply could not be undone.";
    if (button) button.disabled = false;
  }
}

async function deleteCustomerConversation() {
  if (!customerMessagePassword() || !window.confirm("Delete this conversation? This removes it for you and Chey.")) return;
  if (customerDeleteConversation) customerDeleteConversation.disabled = true;
  try {
    await customerMessagesRequest("delete-conversation");
    customerConversation = null;
    renderCustomerMessages();
    if (customerMessagesStatus) customerMessagesStatus.textContent = "Conversation deleted";
  } catch (error) {
    if (customerMessagesStatus) customerMessagesStatus.textContent = error.message || "Conversation could not be deleted.";
  } finally {
    if (customerDeleteConversation) customerDeleteConversation.disabled = !customerMessagePassword();
  }
}
...
async function deleteAdminConversation() {
  ...
  if (button) button.disabled = true;
  try {
    await adminMessagesRequest("delete-conversation", { email: activeAdminMessageEmail });
    activeAdminMessageEmail = "";
    activeAdminConversation = null;
    renderAdminMessageThread();
    await fetchAdminMessages({ showStatus: false });
    if (adminMessagesStatus) adminMessagesStatus.textContent = "Conversation deleted";
  } catch (error) {
    if (adminMessagesStatus) adminMessagesStatus.textContent = error.message || "Conversation could not be deleted.";
    if (button) button.disabled = false;
  }
}
...
function renderAdminMessageThread() {
  ...
  if (!activeAdminConversation) {
    const emptyHeadingKey = "empty-heading";
    if (adminConversationHeading.dataset.renderKey !== emptyHeadingKey) {
      ...
      adminConversationHeading.dataset.renderKey = emptyHeadingKey;
    }
    const emptyKey = "empty|admin";
    if (adminMessageThread.dataset.renderKey !== emptyKey) {
  ...
  if (adminMessageInput) {
    adminMessageInput.disabled = false;
    adminMessageInput.placeholder = "Reply...";
  }
...
async function fetchAdminConversation(email, { showStatus = true } = {}) {
  ...
  try {
    ...
    renderAdminMessageList();
    renderAdminMessageThread();
    if (showStatus && adminMessagesStatus) adminMessagesStatus.textContent = "Live inbox";
  } catch (error) {
    if (showStatus && adminMessagesStatus) adminMessagesStatus.textContent = error.message || "Conversation unavailable.";
  }
}
  ...
  adminCustomerDetail.innerHTML = `
    ...
    <form class="admin-customer-form" id="adminCustomerForm">
      ...
      <label>Account access
        <select data-admin-customer-input="accountStatus"${ADMIN_OWNER_EMAILS.includes(customer.email) ? " disabled" : ""}>
...
function renderAdminTeamDetail() {
  ...
  adminTeamDetail.innerHTML = `
    ...
    <form class="admin-customer-form" id="adminTeamMemberForm">
      <label>Team role<select data-admin-team-input="adminRole"${isOwnerAdmin() && !ADMIN_OWNER_EMAILS.includes(member.email) ? "" : " disabled"}>${member.adminRole === "owner" ? '<option value="owner" selected>Owner</option>' : ""}<option value="admin"${member.adminRole === "admin" ? " selected" : ""}>Admin</option><option value="accountant"${member.adminRole === "accountant" ? " selected" : ""}>Accountant</option><option value="employee"${member.adminRole === "employee" ? " selected" : ""}>Employee</option><option value=""${!member.adminRole ? " selected" : ""}>Remove from team</option></select></label>
      <label>Account access<select data-admin-team-input="accountStatus"${ADMIN_OWNER_EMAILS.includes(member.email) ? " disabled" : ""}><option value="active"${member.accountStatus === "active" ? " selected" : ""}>Active</option><option value="paused"${member.accountStatus === "paused" ? " selected" : ""}>Paused</option><option value="blocked"${member.accountStatus === "blocked" ? " selected" : ""}>Blocked</option></select></label>
...
function orderSearchText(order = {}) {
  return [
    ...
    order.quoteMessage,
    order.adminNote,
    orderSizingSummary(order),
    ...(order.items || []).flatMap((item) => [item.name, item.category, item.shape, item.length, item.sizePreference, item.note])
  ].filter(Boolean).join(" ").toLowerCase();
}

function orderMatchesPaymentFilter(order = {}) {
  const filter = orderPaymentFilter?.value || "all";
  if (filter === "all") return true;
  if (filter === "paid") return order.paymentStatus === "paid";
  if (filter === "quote") return orderHasCustomQuote(order);
...
function filteredLiveOrders() {
  ...
  return liveOrders.filter((order) => {
    ...
    if (search && !orderSearchText(order).includes(search)) return false;
    if (!orderMatchesPaymentFilter(order)) return false;
...
function updateAdminDashboardSummary(orders = liveOrders) {
  const counts = orders.reduce((summary, order) => {
    summary[orderStatusGroup(orderWorkflowStatus(order))] += 1;
    return summary;
...
function adminOrderPageCount(total) {
  return Math.max(1, Math.ceil(total / ADMIN_ORDER_PAGE_SIZE));
}

function adminSelectedOrder(orders = []) {
  if (!orders.length) {
    selectedAdminOrderId = "";
    return null;
  }
  const selected = orders.find((order) => order.id === selectedAdminOrderId);
  if (selected) return selected;
  selectedAdminOrderId = orders[0].id || "";
  return orders[0];
}

function adminOrderRowMarkup(order = {}) {
  const orderId = order.id || "";
  const status = orderWorkflowStatus(order);
  const paid = order.paymentStatus === "paid";
  const paymentLabel = orderPaymentStatusLabel(order);
  const quoteOrder = orderHasCustomQuote(order);
  const isSelected = selectedAdminOrderId === orderId;
  const items = (order.items || []).map((item) => item.name || "Product").filter(Boolean).slice(0, 2).join(", ");
  return `
    <button class="admin-order-row${isSelected ? " is-selected" : ""}${paid ? " is-paid" : " is-pending"}" type="button" data-select-live-order="${escapeAttribute(orderId)}" aria-pressed="${isSelected ? "true" : "false"}">
      <span class="admin-order-row-main">
        <strong>${escapeHTML(orderCustomerName(order))}</strong>
        <small>${escapeHTML(orderId || "Order")} - ${escapeHTML(orderDateSummary(order))}</small>
      </span>
      <span class="admin-order-row-products">${escapeHTML(items || "No products saved")}</span>
      <span class="payment-pill ${paid ? "is-paid" : "is-pending"}">${escapeHTML(paymentLabel)}</span>
      ${quoteOrder ? `<span class="admin-order-mini-pill">Quote</span>` : ""}
      <span class="admin-order-row-status">${escapeHTML(status)}</span>
      <span class="admin-order-row-total">${formatOrderMoney(orderDisplayTotalCents(order), order.currency)}</span>
    </button>
  `;
}
    ...
    <aside class="admin-order-detail-panel ${paid ? "is-paid" : "is-pending"} ${quoteOrder ? "is-quote-order" : ""} ${quoteSentFeedback ? "is-quote-sent-feedback" : ""}" data-order-detail-id="${escapeAttribute(orderId)}">
      <div class="admin-order-detail-head">
        <div>
          ...
          ${quoteSentFeedback ? `<span class="quote-sent-confirmation">Quote accepted and sent</span>` : ""}
          ${order.localSource ? `<span class="quote-sent-confirmation">Recovered from ${escapeHTML(order.localSource)}</span>` : ""}
          <h4>${escapeHTML(orderCustomerName(order))}</h4>
          <p>${escapeHTML(orderId || "Order")} - ${escapeHTML(orderDateSummary(order))}</p>
        </div>
        <strong>${formatOrderMoney(orderDisplayTotalCents(order), order.currency)}</strong>
      </div>
      <div class="admin-live-order-grid">
      ...
      <div class="admin-live-order-actions">
        <label>Status
          <select data-live-order-status="${escapeAttribute(orderId)}">
            ${fulfillmentStatusOptions(status)}
          </select>
        </label>
        ${quoteOrder ? `
          <label>Quote price
            <input data-live-order-quote-amount="${escapeAttribute(orderId)}" inputmode="decimal" placeholder="45" value="${escapeAttribute(quoteCents ? centsToDollars(quoteCents) : "")}" />
          </label>
          <label>Quote status
            <select data-live-order-quote-status="${escapeAttribute(orderId)}">
              ${quoteStatusOptions(order.quoteStatus)}
            </select>
          </label>
          <label>Customer quote message
            <textarea data-live-order-quote-message="${escapeAttribute(orderId)}" placeholder="Example: I can make this as a medium almond set with chrome accents. Accept and pay here when ready.">${escapeTextarea(order.quoteMessage || "")}</textarea>
          </label>
        ` : ""}
        <label>Chey notes
          <textarea data-live-order-note="${escapeAttribute(orderId)}" placeholder="Sizing, make notes, tracking, pickup plan...">${escapeTextarea(order.adminNote || "")}</textarea>
        </label>
...
function renderAdminLiveOrders({ preserveDetail = false } = {}) {
  ...
  const pageCount = adminOrderPageCount(visibleOrders.length);
  adminOrderPage = Math.min(Math.max(adminOrderPage, 1), pageCount);
  const startIndex = (adminOrderPage - 1) * ADMIN_ORDER_PAGE_SIZE;
  const pageOrders = visibleOrders.slice(startIndex, startIndex + ADMIN_ORDER_PAGE_SIZE);
...
function setLiveOrderFilterStatus(prefix = "Showing") {
  ...
  const suffix = visibleCount === liveOrders.length ? "" : " after filters";
...
function collapseVisibleLiveOrders() {
  adminOrderPage = 1;
  renderAdminLiveOrders();
  setLiveOrderFilterStatus("Showing newest page of");
}

function expandVisibleLiveOrders() {
  adminOrderPage = adminOrderPageCount(filteredLiveOrders().length);
  renderAdminLiveOrders();
  setLiveOrderFilterStatus("Showing oldest page of");
}
...
function handleAdminLiveOrderClick(event) {
  ...
  if (pageButton) {
    const visibleOrders = filteredLiveOrders();
    const pageCount = adminOrderPageCount(visibleOrders.length);
    adminOrderPage += pageButton.dataset.orderPage === "next" ? 1 : -1;
    adminOrderPage = Math.min(Math.max(adminOrderPage, 1), pageCount);
    const nextPageOrder = visibleOrders[(adminOrderPage - 1) * ADMIN_ORDER_PAGE_SIZE];
    selectedAdminOrderId = nextPageOrder?.id || selectedAdminOrderId;
    renderAdminLiveOrders();
    setLiveOrderFilterStatus("Showing");
    return;
  }
  const selectButton = event.target.closest("[data-select-live-order]");
  if (selectButton) {
    selectedAdminOrderId = selectButton.dataset.selectLiveOrder || "";
    renderAdminLiveOrders();
    return;
  }
...
setupMessageEmojiPicker(customerEmojiButton, customerEmojiPicker, customerMessageInput);
setupMessageEmojiPicker(adminEmojiButton, adminEmojiPicker, adminMessageInput);
...
adminMessageThread?.addEventListener("click", (event) => {
  const button = event.target.closest("[data-undo-message]");
  if (button) undoAdminMessage(button.dataset.undoMessage, button);
});
adminConversationHeading?.addEventListener("click", (event) => {
...
orderStatusFilter?.addEventListener("change", () => applyLiveOrderDateFilters());
